...
如需对中英文正文中的无依据分析尾句、宣传性评价、模糊归因、间接谓词、同义词循环、
虚假范围与空泛 outlook 进行 claim-local 审阅，请渐进加载
[DEAI_PATTERN_CLUSTERS.md](DEAI_PATTERN_CLUSTERS.md)。这些 `[LLM]` 提示不用于判断 AI
作者身份，也不得改造成词表或检测器分数规则。
...
3. **无立场的讨论**——列出各方利弊，却不作判断。证据能够区分备选解释时，应说明哪一项更可信及其理由；证据不能区分时，应明确说明机制尚未确定，不能为了显得有立场而强行选边。
4. **统一的段落长度** — 80% 的段落由 5-7 个句子组成。真正的节奏各不相同：一个 3 句话的强调段落，旁边是一个 10 句话的论证。
5. **防御性推测解释**——观察结果后连续列出两个及以上具体机制，却没有逐机制证据或区分性检验，最后再用总括性限制语声明当前数据无法验证其中任何一项。`may`、`could`、`可能` 或 `与……一致` 只能校准措辞强度，不能提供证据。

**如何修复**：打破对称——合并过薄段落、拆分过载段落并删除声明式过渡。处理候选解释时，先陈述观察结果，再把保留的每项机制绑定到可见指标、图表、消融、受控对比、引用或区分性检验。只有证据能够区分备选解释时才明确选择；否则说明机制尚未确定，并将可检验的备选解释移入未来工作。不得为了显得果断而删除限制语或强化无证据支持的解释。

脚本的 `hedge` 和 `hedge_application` 建议仍适用于过度自信措辞和未演示应用。
加入 `results suggest`、`may / could` 或 `可能/或许` 只能降低论断强度，不能许可
无证据的机制堆叠。
...
**指南**：
- 解释机制，而不仅仅是结果，并把保留的每项机制绑定到可见证据
- 将归因词视为 `[LLM]` 复核线索，而不是解释已获支持的证明
