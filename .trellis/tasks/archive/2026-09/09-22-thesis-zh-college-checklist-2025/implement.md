...
1. 先用research/checklist-map逐项核对并落111行新模板；doctor/master分开测试；旧四模板与八组调用基线冻结。
   先添加本子AC矩阵中的失败/反例测试；阶段验证用下面目标命令按对应test名称运行。
2. 按design§3新增third_person，§4追加学院MODULE命令提示；不新增PDF入口/阈值，复合项保持人工分流。
...
rtk proxy uv run --extra dev python -m pytest -q tests/skills/latex_thesis_zh/test_check_spec.py tests/skills/latex_thesis_zh/test_latex_thesis_zh_coverage.py tests/contracts/test_spec_checklists.py
