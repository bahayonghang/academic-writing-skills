"""Recount core-rule evidence from complete observations."""
...
OBS = ELS_ROOT / "observations"
KNOW = ELS_ROOT / "knowledge"
...
def rewrite_rules(papers: list[tuple[str, str]]) -> None:
    ...
    if not rows:
        print("writing_rules empty")
        return
    header = list(rows[0].keys())
    counts = {
        "R002": keys_matching(papers, r"story_pattern:\s*SP-ELS-002"),
        "R003": keys_matching(papers, r"The rest of this (paper|article) is organized as follows"),
        "R004": keys_matching(
            papers,
        ...
        "R006": keys_matching(papers, r"story_pattern:\s*SP-ELS-001"),
        "R009": keys_matching(papers, r"this (paper|article) proposes", ignore_case=True),
    }
    for row in rows:
...
def rewrite_domain() -> None:
    ...
    for item in payload["items"]:
        if item.get("status") != "complete":
            continue
        counts[item.get("venue") or "(no venue)"] += 1
    ...
    for venue, n in sorted(counts.items(), key=lambda kv: (-kv[1], kv[0])):
        lines.append(f"{venue}\t{n}\t\tcomplete observations\n")
