...
运行 `deep-review` 通道任务时使用这些模板。

## 执行方式

这些模板定义输入、只读文件范围、输出路径和出处。它们不要求名为 `Task` 的工具。

- 输入：模板中的 `Read:` 列表，以及匹配的 Focus / DO / DON'T 块。
- 文件范围：只读取 `<review_dir>` 下列出的文件和点名的参考文件。不要修改论文源码。
- 输出：把 JSON 数组写入 `<review_dir>/comments/<lane_name>.json`（或该通道 Output limit 中的路径）。每条 finding 保留 `[Script]` 与 `[LLM]` 出处。
- `native delegated`：仅当当前工具确实创建了独立子代理时，为每个独占通道生成一个子代理，并在这些独占范围上并行。
- `sequential single-agent`：当前会话没有原生委派时，在同一代理内按通道逐个运行相同模板。保留相同输出。不要把该运行描述为独立专家组。
