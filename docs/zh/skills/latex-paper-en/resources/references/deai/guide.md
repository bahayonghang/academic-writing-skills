...
如需按 claim-local 方式审阅无依据分析尾句、宣传性评价、模糊归因、间接谓词、同义词循环、
虚假范围与空泛 outlook，请渐进加载 [pattern-clusters.md](pattern-clusters.md)。这些 `[LLM]`
提示不用于判断 AI 作者身份，也不得改造成词表或检测器分数规则。
...
5. **防御性推测解释**——观察结果后连续列出两个及以上具体机制，却没有逐机制证据或区分性检验，最后再用总括性限制语声明当前数据无法验证其中任何一项。`may`、`could` 或 `is consistent with` 只能校准措辞强度，不能提供证据。
...
加入 `results suggest` 或 `may / could` 只能降低论断强度，不能许可无证据的机制堆叠。
