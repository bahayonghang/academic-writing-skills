# 超额声明保护

Typst 论文的保守措辞参考（英语/双语）。目标是
**不是**胆怯的散文——它是准确地陈述证据强度：有力的证据值得
措辞强硬，证据薄弱，措辞也薄弱。

## 范围

本指南决定**一旦知道证据强度后如何表达主张** - 这
动词/限定词的选择，这样句子就不会超出证据范围。解决证据
充分性第一（可见的数字/指标/引文是否支持该主张），
然后在这里选择措辞。当实质内容和措辞似乎发生冲突时，实质内容胜出。

## 确定性阶梯（动词，最强 → 最弱）

```
demonstrate / prove                         ← intervention + controlled experiment
  ↓
reveal / identify / find                    ← strong effect, multi-method or replicated
  ↓
indicate / suggest                          ← significant but single-method
  ↓
support / be consistent with                ← trend, agrees with prior work
  ↓
may indicate / could suggest / appear to    ← marginal or predictive
  ↓
hint at / point toward                      ← very weak signal or hypothesis
```

选择与证据相匹配的梯级。不要爬上数据无法到达的梯级。

## 替换表

### 1.因果关系（最常见的过度主张：相关性被表述为因果关系）

|❌ 过度主张|✅ 保守|
|---|---|
|引起的|与/链接到|
|驱动/驾驶|有助于/与...相关|
|决定|影响/形状|
|负责|涉及/与...有关|
|结果|随后/同时出现|
|证明|表明/提供证据表明|

仅在受控干预（消融、随机
作业、A/B 测试）、工具变量设计或已经建立的
您的数据重现机制。否则使用关联措辞。

### 2.新颖性/首创性（审稿人在几秒钟内验证这些）

|❌ 过度主张|✅ 保守|
|---|---|
|第一个到|据我们所知，第一个/第一个|
|小说（自有标签）|说出新内容；放下标签|
|空前的|实质性的/显着的|
|以前未知的|没有广泛研究|

### 3. 普遍性（研究一种设置，声明所有设置）

|❌ 过度主张|✅ 保守|
|---|---|
|总是/从不|一般/很少|
|在所有情况下|在研究的案例中|
|普遍地|跨越评估的基准|
|任何数据集|采样的数据集|

### 4. 效应大小（没有数字的模糊大小词）

|❌ 过度主张|✅ 保守|
|---|---|
|强有力的改进|误差减少 X%|
|作用大|β = X.XX（95% CI：……）|
|显着的收益|从 X 改进到 Y (p = ...)|
|非常重要的|p < 1 × 10⁻1⁰|
|强壮的|在 N 次运行中保持一致/在[扰动]下稳定|

如果数字本身就有分量，那就去掉形容词——数字会说话。

### 5. 时间/推断顺序（当前数据，过去机制）

|❌ 过度主张|✅ 保守|
|---|---|
|X 推动了变革|变化与X一致|
|发生在时间 T|估计表明〜T（CI：...）|
|从A迁移到B|数据符合路径A→B|

### 6. 应用/影响（此处未展示下游用途）

|❌ 过度主张|✅ 保守|
|---|---|
|将彻底改变|有潜在影响|
|将被广泛使用|可能有用/可以告知|
|解决了X的问题|解决 X 的一方面|
|准备部署|为[设置]提供了一种候选方法|

### 7.比较（贬低以前的工作）

|❌ 过度主张|✅ 保守|
|---|---|
|以前的方法都失败了|以前的方法受到以下限制|
|优于所有先前的工作|与【具体方法】相比毫不逊色|
|解决了长期存在的争论|为辩论的一方添加证据|

## 高频陷阱短语

|陷阱|安全更换|
|---|---|
|“我们的结果证明了 X。” （X 因果）|“我们的结果与 X 一致。”|
|“这是第一个作品……”|“据我们所知，最先……”|
|“X 在 Y 中起着至关重要的作用。”|“X 与 Y 有牵连/可能对 Y 有贡献。”|
|“这些发现对......具有重要意义”|“这些发现为进一步研究……提供了基础”|
|“X 是 Y 的关键驱动力。”|“X 与 Y 相关。”|
|“强烈支持”|“符合/提供符合的证据”|

## 反向校准：何时不进行对冲

对冲弱证据是正确的；对冲有力的证据是胆怯。在以下情况下使用强硬措辞：

- 受控干预（消融/随机对照试验/A-B）给出因果结果 → `demonstrate`；
- 多种方法/数据集/种子复制结果 → `robustly`，并命名证据；
- 再现既定机制→`confirms` / `validates`;
- 强大的统计效果→强大的措辞**加上**数字。

## 自检（起草一段后扫描）

- [ ] 用过的`first` / `novel`？您是否真的搜索过文献，或者添加了“据我们所知”？
- [ ] 用过的`cause` / `drive` / `determine`？有干预吗？如果没有→`associated with`.
- [ ] 用过的`all` / `always` / `universally`？范围是否限于您所学的内容？
- [ ] 用过的`significant` / `strong` / `substantial`？附有号码吗？
- [ ] 列出的含义您没有证明？添加`may` / `could`.
- [ ] 贬低之前的工作？重新定义为“受限于”，而不是“失败”。

## 脚本支持

`deai_check.py` 标记一组明确的过度声明短语（因果/
首创性/通用性/应用）作为 `[Script]` 低跟踪并指向
本指南。该脚本是针对明显情况的安全网；上面的表格涵盖了
它无法做出判断。
