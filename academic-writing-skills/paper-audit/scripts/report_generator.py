...
def render_review_report(result: AuditResult) -> str:
    ...
    lines = [
        ...
        f"**File**: `{result.file_path}` | **Language**: {result.language.upper()}",
        f"**Generated**: {now}"
        + (f" | **Target Venue**: {result.venue}" if result.venue else ""),
        "",
    ]
    ...
    if result.issues:
        ...
        for issue in sorted(result.issues, key=lambda i: ("Critical", "Major", "Minor").index(i.severity) if i.severity in ("Critical", "Major", "Minor") else 3):
            loc = f"(Line {issue.line}) " if issue.line else ""
            lines.append(
                f"- **[{issue.module}]** {loc}[{issue.severity}]: {issue.message}"
            )
        lines.append("")
