...
1. 核对design主题表；逐个指南补五主题正反例、归因与人工边界；保留历史保护句。
   先添加本子AC矩阵中的失败/反例测试；阶段验证用下面目标命令按对应test名称运行。
2. 按design§3补触发/Reference Map和方法8码文档；不改scripts/词表，核对CF/T码集不变。
...
rtk proxy uv run --extra dev python -m pytest -q tests/contracts/test_thesis_zh_guidance_fidelity.py tests/contracts/test_claim_forward_contract.py
