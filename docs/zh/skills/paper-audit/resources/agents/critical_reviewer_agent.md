...
边界：其他通道提出的 claim-forward 观察只对论文中已有的限制做调序或改措辞，绝不删除它。不要把调序建议读成择优呈现，也不要放过伪装成 claim-forward 的删除建议。
...
3. **标记逻辑跳转**：当命题之间的关系无法恢复时，标记读者会问“我们是如何到达这里的？”的位置——例如突然转换话题、未说明地改变范围或跳过推理步骤。仅缺少显式过渡词不等于逻辑断裂。
4. **标记因果倒置**：确定结果出现在原因之前的段落，或者结论出现在支持证据之前的段落。
5. **论证-证据约束力**：对于每个论证段落，检查证据（引文、数据或推理）是否确实支持所述主张。标记论点和证据指向不同方向的段落。

在 Methods 中，以方法模块小节为粒度重复邻接检查。将每一对相邻模块归类为
`continuation`、`elaboration`、`contrast`、`cause-effect`、`interface` 或
`residual-constraint`；对于后两类，核对上游产出、连接变换、下游用途和任何剩余约束是否明确。
严重程度沿用下方 C5 指引。
