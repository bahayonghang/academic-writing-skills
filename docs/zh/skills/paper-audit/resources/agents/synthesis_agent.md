...
|量词|定义|使用案例|
| ---------- | ------------------------------------------------------------ | ------------------------------------------------ |
| `any`      |谓词适用于 >= 1 个审阅者/通道|标记孤立的门禁阻断发现|
...
- 哪些审查通道报告了此事
- 每条审查通道的严重程度（`major | moderate | minor`）；CRITICAL 级输入由整合过程归一化为 `major + gate_blocker=true`
...
- 如果严重性跨度 >= 2 级，或者一条通道报告 `gate_blocker=true` 而其他通道报告轻微问题，
...
- `gate_blocker=true` 问题会阻断 `gate` 模式，并成为路线图中的优先级 1
