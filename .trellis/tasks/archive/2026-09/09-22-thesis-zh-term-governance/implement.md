...
1. 先采集consistency默认/terms/abbreviations/旧JSON与style基线；按design§2完成governance局部扫描和JSON入口。
   先添加本子AC矩阵中的失败/反例测试；阶段验证用下面目标命令按对应test名称运行。
2. 按design§3完成abbreviation-style，§4完成degree-wording；保护输入、源定位、重复候选去重；仅更新style授权哈希。
...
rtk proxy uv run --extra dev python -m pytest -q tests/skills/latex_thesis_zh/test_term_governance.py tests/skills/latex_thesis_zh/test_consistency_semantics.py tests/skills/latex_thesis_zh/test_check_style_zh.py tests/skills/latex_thesis_zh/test_polish_unit_zh.py
