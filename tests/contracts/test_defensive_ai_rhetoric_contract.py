...
def test_each_surface_has_a_local_composite_fixture_and_eval(
    ...
    matches = [
        ...
        if item.get("id") == defensive_eval_id and fixture_rel in item.get("files", [])
    ]
    ...
    assert eval_ids == sorted(eval_ids), f"{skill}: eval ids must remain append-only"
