...
```powershell
rtk uv run python docs/scripts/check_resource_sync.py --skill typst-paper
...
- `check_resource_sync.py --skill typst-paper`: passed, 250-entry manifest loaded.
- `pytest tests/contracts/test_docs_bilingual_resources.py -q`: 8 passed.
- `npm --prefix docs run docs:build`: passed with VitePress 1.6.4 after correcting 14
  migrated relative links.
- Staged `git diff --check`: passed after Windows line-ending normalization.
- High-risk bilingual samples reviewed: routing notes, Typst syntax, PSEUDOCODE/REFERENCES/DEAI
  modules, and IEEE template.
