...
1. 采集references、verify_bib三standard、literature普通/intro-citations基线；按design§2–§3在references做支持语法和位置/页码候选。
   先添加本子AC矩阵中的失败/反例测试；阶段验证用下面目标命令按对应test名称运行。
2. 按design§4接入college-details和progression-density；姓名只作样式说明，缺页码不编造，边界外语法报告未覆盖。
...
rtk proxy uv run --extra dev python -m pytest -q tests/skills/latex_thesis_zh/test_citation_literature.py tests/skills/latex_thesis_zh/test_verify_bib_scanner.py tests/skills/latex_thesis_zh/test_latex_thesis_zh_coverage.py
