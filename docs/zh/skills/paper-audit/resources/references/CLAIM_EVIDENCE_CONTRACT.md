...
主张与证据的不匹配有两个方向。当证据行已经支撑比原文更强的陈述时，`allowed_wording` 可以比原文措辞更强，但只抬到锚点已支撑的那一级；作为 `claim_accuracy` 报告，并遵循 `OVER_CLAIM_GUARD.md` 的低估主张一节。绝不用删除限制语来换取更强的措辞。

## 防御性推测解释

存在以下组合时，将多机制解释视为 `unsupported extrapolation`：

- 从观察结果出发，却列出两个或更多具体机制；
- 至少一项机制缺少可见证据锚点或区分性检验；并且
- 段尾限制语声明当前数据或比较无法验证其中任何一项。

`may`、`could` 和 `is consistent with` 等限定语只能校准措辞，不能创造证据。保留观察结果，把保留的每项机制分别映射到自己的证据锚点，并将无支持的备选项标为 `speculative` 或 `undetermined`。如果没有任何机制得到支持，应说明机制尚未确定并记录缺少的实验或分析；不得删除限制语或强化推断。
