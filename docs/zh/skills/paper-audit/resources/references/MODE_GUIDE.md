...
- 首先从请求推断输出语言，然后再回到论文
当请求不明确时的语言。
- 对于 `re-audit`，需要 `--previous-report PATH`。缺少该参数时，先按
  `*audit_report*` / `*review_report*` /
  `*final_issues*.json` 模式在论文目录和当前工作目录中查找候选。
  恰好一个候选时，陈述解析出的路径并继续。零个或多于一个候选时，
  立即停止，只询问该路径，而不是运行新的审核。
...
从不未经确认自动切换模式。目标是在运行错误的工作流程之前捕获明显的模式
不匹配——而不是重复询问用户已经回答过的问题。

下面每一项检测都按模式的设定方式分支：

- **模式由用户明确给出**：该决策用户已经做出。把检测到的信号作为一句陈述
  报告，并按用户给出的模式运行。不要给出模式选择题。唯一的例外是实质冲突
  （定义见下）。
- **模式由你推断得出**：尚未做出决策，因此提问。
- **出现实质冲突，两种情况均提问**：提问。

只有当按给定模式执行会改变审查范围或改变结果时，检测才算实质冲突：

- **范围**：必须增加或删除章节，或有新的对比基线进入审查（用户要求对比的
  前次报告、审稿信）。
- **结果**：`gate` 的 PASS/FAIL 判定改变，或某个问题的严重度改变。
- **交付级别**：给定模式无法在用户同时给定的交付级别下运行——例如 `deep-review`
  搭配 `T3`。说明是哪个模式与哪个级别冲突，然后给出降级模式及其能力差距。
  参见 `Delivery Boundary` 一节，位于 `SKILL.md`。

本次运行不会读取的陈旧产物不构成实质冲突。
正例——用户要求 `quick-audit`，而 `audit_report_2026-08.md` 就在论文旁边：
旧报告不是 `quick-audit` 的输入，因此范围和结果都不变。陈述“在论文旁边发现
`audit_report_2026-08.md`；按要求运行 `quick-audit`”，然后继续。反例——用户
要求 `quick-audit` 但存在审稿信：审稿信增加审查范围，因此提问。

- **以前的报告存在**：如果文件名为 `*audit_report*`、`*review_report*`、
  `*final_issues*.json`，或语义上匹配 `--previous-report` 的文件存在于论文
  目录或当前工作目录，询问用户是否想要 `re-audit` 模式。如果用户已经给出了
  模式，用一句话点名该文件，并保持该模式。
- **论文中的修订标记**：如果来源包含 `\latextrackchanges`、`changes` 包宏、
  `track-changes`、`changeBars`、`\added{`、`\deleted{`、`\replaced{`、
  `<changes>`，或 `Revision History` 部分，询问这是否是修订后的提交，以及
  是否有意使用 `re-audit`。如果用户已经给出了模式，用一句话报告这些标记，
  并保持该模式。
- **长论文上的抛光模式**：如果模式为 `polish` 但论文超过 30 页或 25k 字，
  在继续之前询问 `deep-review` 是否更合适。如果用户明确给出了 `polish`，
  用一句话报告篇幅，并保持 `polish`。
- **检测到审稿人信件**：如果输入或工作目录包含审稿信形状的文件（标记：
  `Reviewer 1`、`R1:`、`审稿人 1`、`Editor's Comments`、`Decision Letter`），
  先派遣 `agents/revision_coach_agent.md` 将其解析为结构化路线图，然后把
  路线图输入 `re-audit`。审稿信总是增加审查范围，因此这属于实质冲突：
  即使用户给出了其他模式也要提问。

始终以简单语言呈现检测到的信号（“在论文旁边发现
`final_issues.old.json`——这看起来像是重新审核”）。
当用户已给出模式且不存在实质冲突时，这句话就是对该信号的全部响应——
不加确认步骤直接继续。否则，让用户确认或拒绝。
