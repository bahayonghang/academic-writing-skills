...
## Entry / dependencies
C1完成；接手style；交付后references移交C3，CLI交给C5。
...
1. 采集style/format/tables/references旧路径基线；按design§2接入四入口school参数，再做§3数字单位与分组候选。
   先添加本子AC矩阵中的失败/反例测试；阶段验证用下面目标命令按对应test名称运行。
2. 按design§4公式raw扫描和§5双语题注/表身边界实现；不改旧模板；覆盖C1组合开关并更新style哈希。
...
rtk proxy uv run --extra dev python -m pytest -q tests/skills/latex_thesis_zh/test_number_equation_table.py tests/skills/latex_thesis_zh/test_check_style_zh.py tests/skills/latex_thesis_zh/test_polish_unit_zh.py tests/skills/latex_thesis_zh/test_latex_thesis_zh_coverage.py
